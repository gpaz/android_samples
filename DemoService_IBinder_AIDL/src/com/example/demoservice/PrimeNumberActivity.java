...
import com.example.demoservice.aidl.IPrimeNumberGenerator;
...
public class PrimeNumberActivity extends Activity implements OnClickListener
{
    private static final String TAG = PrimeNumberActivity.class.getSimpleName();
    private static final String KEY_PRIME_NUMBERS = "prime-numbers";
    
    private IPrimeNumberGenerator mBoundService;
    ...
    private ServiceConnection mServiceConnection = new ServiceConnection()
    {
        ...
        {
            mBoundService = IPrimeNumberGenerator.Stub.asInterface(service);
    ...
    private class PrimeNumberServiceProvider implements IInterfaceProvider<IPrimeNumberGenerator>
    {
        /***************************** getService *****************************/
        @Override
        public IPrimeNumberGenerator getProvider()
        {
    ...
    private static class GetPrimeNumbersTask extends AsyncTask<Number, Void, long[]>
    {
        ...
        private IInterfaceProvider<IPrimeNumberGenerator> mServiceProvider;
        
        /************************** CONSTRUCTOR *******************************/
        public GetPrimeNumbersTask(View interactionView, TextView displayView,
                IInterfaceProvider<IPrimeNumberGenerator> serviceProvider)
        {
        ...
        {
            long[] results = EMPTY_LONGARRAY;
            if(params.length == 3)
            {
                final long from = (Long)params[0];
                final long to = (Long)params[1];
                final int count = (Integer)params[2];
                IPrimeNumberGenerator service = mServiceProvider.getProvider();
                if(service != null)
                {
                    try
                    {
                        results = service.generatePrimeNumbers(from, to, count);
                    }
